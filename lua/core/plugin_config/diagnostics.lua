vim.notify("✅ diagnostics.lua loaded successfully!", vim.log.levels.INFO)
local M = {}
...
function M.setup(bufopts)
  ...
  vim.keymap.set("n", "<Space>dn", M.next, vim.tbl_extend("force", bufopts, { desc = "Next diagnostic (by severity)" }))
  vim.keymap.set("n", "<Space>dp", M.prev, vim.tbl_extend("force", bufopts, { desc = "Previous diagnostic (by severity)" }))
  vim.keymap.set("n", "<Space>de", M.open_float, vim.tbl_extend("force", bufopts, { desc = "Show diagnostic under cursor" }))
